---
title: Overview
sidebar_label: "📘 Overview"
sidebar_position: 1
---
import {
  HomepageCard as Card,
  HomepageSection as Section,
} from '../../src/components/HomepageComponents';

import {
  Network,
  Nodes,
  Guide,
  P2pCoin,
} from '../../src/icons';

# Sentinel dVPN Nodes

A comprehensive look into the Sentinel dVPN Nodes, where we'll break down what they do, why they're important, and how you can make money through subscription plans and health checks. This exploration will give you a thorough understanding of these nodes and the opportunities they present.

<Section id="web-sdks" hasSubSections >
  <Section>
    <Card
      title="Introduction"
      description="Introducing the dVPN Node: understanding its purpose, bandwidth sharing concept, and crucial role within the Sentinel Ecosystem"
      to="/dvpn-nodes/intro"
      icon={<Nodes />}
    />
    <Card
      title="Run a dVPN Node"
      description="Discover the process of setting up a dVPN node to contribute to the Sentinel network by sharing your bandwidth with this comprehensive step-by-step guide"
      to="/dvpn-nodes/setup"
      icon={<Guide />}
    />
    <Card
      title="Earnings"
      description="Hosting a dVPN node incurs costs such as server expenses and electricity bills. However, these expenses can be offset through the Node Revenue System, allowing for monetization of the service"
      to="/dvpn-nodes/earnings"
      icon={<P2pCoin />}
    />
    <Card
      title="Health Check"
      description="Successfully completing the Health Check is the primary prerequisite for eligibility to earn node rewards and monetize your node. Here's a guide on how to accomplish it"
      to="/dvpn-nodes/health-check/overview"
      svgFile="/icons/health-check.svg"
    />
    <Card
      title="Reference & Tools"
      description="Explorers, dashboards, faucets, and monitoring bots for the Sentinel Node Network, built by both the Sentinel team and the community"
      to="/dvpn-nodes/tools"
      svgFile="/icons/stats.svg"
    />
    <Card
      title="RPC List"
      description="dVPN nodes rely on RPC nodes to reach the blockchain. Find up-to-date public RPC endpoints and pick a reliable, nearby one for your node"
      to="/dvpn-nodes/rpc-list"
      icon={<Network />}
    />
  </Section>
</Section>