...
An interactive web UI for all the gRPC and Protobuf services is stored on the [Sentinel Buf Schema Registry](https://buf.build/trinityvalidator/sentinel-dvpn)

Studio preconfigured with the mainnet RPC:
- [Launch Buf Studio on mainnet RPC](https://buf.build/studio/trinityvalidator/sentinel-dvpn/?target=http%3A%2F%2Fgrpc.sentinel.co%3A9090)
